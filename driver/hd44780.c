...
HD44780_Result hd44780_init(HD44780 *display, HD44780_Mode mode, const HD44780_Config *config, uint8_t columns, uint8_t rows, HD44780_CharSize charsize)
{
  ...
  HD44780_RETURN_ASSERT(columns > 0 && columns <= HD44780_COLUMNS_AMOUNT_MAX, HD44780_RESULT_ERROR);
  HD44780_RETURN_ASSERT(rows    > 0 && rows    <= HD44780_LINES_AMOUNT_MAX,   HD44780_RESULT_ERROR);
...
HD44780_Result hd44780_move_cursor(HD44780 *display, uint8_t column, uint8_t row)
{
  ...
  if(row < 2)
    ...
    // Универсальная коррекция для 4х-строчных дисплеев, вне зависимости от длины строки (16 или 20 символов): 
    // нижние строки, начиная с третьей - используют начальное смещение первых двух строчек, плюс длина строки... 
    // (Потому что видеопамять распределяется непрерывно: "первая строка" продолжается в "третья строка", "вторая строка" продолжается в "четвертая строка".)
    row_offset = row_offsets[row%2] + display->columns_amount;

  return hd44780_command(display, HD44780_CMD_SETDDRAMADDR | (column + row_offset));
}
...
HD44780_Result hd44780_read_bits(HD44780 *display, uint8_t *value)
{
  ...
  for (unsigned i = 0; i < display->dp_amount; ++i)
  {
    ...
    HD44780_RETURN_IF_ERROR(gpios->read(gpios, (HD44780_Pin) (display->dp_first + i), (HD44780_PinState*) &bit));
